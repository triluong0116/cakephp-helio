...
class LandToursController extends AppController
{
    ...
    public function lists()
    {
        ...
        if ($check['status']) {
            ...
            foreach ($landtours as $landtour) {
                ...
                $location = $this->Locations->find()->where(['id' => $landtour['destination_id']])->first();
                $data = [];
                $data['id'] = $landtour['id'];
                $data['name'] = $landtour['name'];
                $data['thumbnail'] = $landtour['thumbnail'];
                $data['rating'] = $landtour['rating'];
                $data['singlePrice'] = $landtour['singlePrice'];
                $data['location_id'] = $location['id'];
                $data['location_name'] = $location['name'];
                $listLandtour[] = $data;
            }

            $listLandtour = array_slice($listLandtour, 10 * ($page - 1), 10);
            $this->set([
                'status' => STT_SUCCESS,
                'message' => 'Success',
                'data' => $listLandtour,
    ...
    public function calPrice()
    {
        ...
        Log::write('debug','Check calPrice Lantour: '.json_encode($data));
        $this->set([
            'status' => STT_SUCCESS,
            'message' => 'Success',
            'data' => $response,
...
//        dd($data);
//        Log::write('debug', 'data_callPriceBooking:"' . json_encode($data, JSON_UNESCAPED_SLASHES) . '"');
        ...
        if ($data['booking_landtour']['num_adult']) {
            ...
            $pick_up = [];
            foreach ($data['drive_location_id'] as  $key => $value) {
                foreach ($landtour->land_tour_drivesurchages as $item) {
                    if ($value == $item['id']) {
                        $pick_up[] = trim( $item->name);
                    }
                }
            }
