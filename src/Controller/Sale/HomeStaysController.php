...
class HomeStaysController extends AppController
{
    ...
    public function index()
    {
        ...
        if ($this->request->is('get') && $this->request->getQuery('search')) {
            ...
            $list_object_homestay = $this->HomeStays->find()->where([
                'OR' => [
...
//        dd($homestay);
    ...
    public function setFeatured() {
        ...
        $data = $this->request->getData();
        if ($data['ids']) {
            $this->HomeStays->updateAll(['is_feature' => 1], ['id IN' => $data['ids']]);
        }
    ...
    public function unsetFeatured() {
        ...
        $data = $this->request->getData();
        if ($data['ids']) {
            $this->HomeStays->updateAll(['is_feature' => 0], ['id IN' => $data['ids']]);
        }
        $response['success'] = true;
    ...
    public function convertDescription(){
        ...
        foreach ($homestays as $homestay){
           $description = strip_tags($homestay->description);
            $caption[0]=[
                'content'=>$description
         ];
           $homestay=$this->HomeStays->patchEntity($homestay,['caption'=>json_encode($caption)]);
