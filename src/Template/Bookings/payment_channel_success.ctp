<div class="bg-grey">
    ...
    <div class="payment-detail pt20 pb20">
        <div class="container">
            <div class="bg-white p20">
                    <?php if ($booking->channelbooking_rooms): ?>
                        <?php foreach ($booking->channelbooking_rooms as $roomKey => $booking_room): ?>
                            <fieldset class="booking-room-item">
                                <legend>Hạng phòng <?= $roomKey + 1 ?></legend>
                                <div class="table-responsive">
                                    <table class="table">
                                        <tr>
                                            <td><h4>Tên phòng: <?= $booking_room['channel_name'] ?></h4></td>
                                        </tr>
                                        <?php foreach ($booking_room['packages'] as $package): ?>
                                            <tr>
                                                <td class="bold">Phòng: <?= $package->channelroom_code ?></td>
                                            </tr>
                                            <tr>
                                                <td>Mã gói: <?= $package->channel_package_code ?></td>
                                            </tr>
                                            <tr class="pc">
                ...
                <?php
                $listMemberRoom = json_decode($booking->channel_information, true);
                ?>
        ...
        <div class="container ">
            <div class="bg-white p20" style="min-height: 200px">
                <?php if ($payment->type == PAYMENT_ONEPAY_CREDIT || $payment->type == PAYMENT_ONEPAY_ATM || $payment->type == PAYMENT_ONEPAY_QR): ?>
                    <h3 class="text-light-blue">Mã đơn hàng : <span class="text-dark"><?= $booking->reservation_id ?></span></h3>
                <?php else: ?>
    ...
    <div class="container pb20 mb20">
        <div class="bg-white pt50 pb50 ">
            <p style="text-align: justify" class="main-color fs20 pl30 pr30 pt10"> Cảm ơn bạn đã sử dụng hệ thống  Booking Online của Mustgo. Chúng tôi đã nhận được đơn hàng và Ủy nhiện chi thanh toán. Mustgo sẽ
                gửi đặt phòng qua  tại thời điểm xác nhận tiền nổi trên tài khoản Mustgo. Tình trạng phòng có thể hết trước khi Mustgo Xác định tiền nổi trên tài khoản. Quý khách vui lòng liên
