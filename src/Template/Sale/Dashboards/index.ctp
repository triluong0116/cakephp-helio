            ...
            <?php foreach ($bookinsByMonth as $monthKey => $singleMonth): ?>
                <div class="x_panel">
                    <div class="x_title">
                        ...
                        <?php $totalProfit = 0; ?>
                        <?php
                        if (isset($singleMonth['booking']) && !empty($singleMonth['booking'])) {
                            foreach ($singleMonth['booking'] as $key => $booking) {
                                if ($booking->sale_id != $booking->user_id) {
                                    $totalProfit += $booking->sale_revenue;
                                } else {
                                    if (strtotime($booking->complete_date) < strtotime('2022-05-01')) {
                                        $totalProfit += $booking->sale_revenue + $booking->revenue - $booking->agency_discount;
                                    } else {
                        ...
                        if (isset($singleMonth['vin_booking']) && !empty($singleMonth['vin_booking'])) {
                            foreach ($singleMonth['vin_booking'] as $key => $booking) {
                                if ($booking->sale_id != $booking->user_id) {
                                    $totalProfit += $booking->sale_revenue - $booking->agency_discount - $booking->sale_discount;
                                } else {
                                    $totalProfit += $booking->sale_revenue + $booking->revenue - $booking->agency_discount - $booking->sale_discount;
                                }
                            }
                        }
                        ?>
                                    ...
                                    <table class="table">
                                        ...
                                        <?php foreach ($singleMonth['booking'] as $key => $booking): ?>
                                            ...
                                            <tr>
                                                ...
                                                <td><?= number_format($booking->total_price - $booking->sale_revenue - $booking->revenue) ?></td>
                                                <td><?php
                                                    if ($booking->sale_id != $booking->user_id) {
                                                        echo number_format($booking->total_price - $booking->revenue);
                                                    } else {
                                                        echo number_format($booking->total_price);
                                                    }
                                                    ?></td>
                                                <td>
                                                    <?php
                                                ...
                                                <td>
                                                    <?= $booking->mail_type == 1 ? 'Đã TT' : 'Công nợ' ?>
                                                </td>
                                                <td>
                                                    <?= $booking->pay_hotel_type == 1 ? 'Đã TT' : 'Công nợ' ?>
                                                </td>
                                                <td class="actions">
                    ...
                    <?php endif; ?>
                    <?php if (isset($singleMonth['vin_booking'])): ?>
                        <div class="row mt15">
                            <div class="col-sm-12 col-md-12 col-xs-12">
                                <div class="x_content table-responsive" style="display: none">
                                    <h3>Danh sách Booking Vin</h3>
                                    <table class="table">
                                        <thead>
                                        <tr>
                                            <th scope="col">#</th>
                                        ...
                                        <tbody>
                                        <?php foreach ($singleMonth['vin_booking'] as $key => $booking): ?>
                                            <?php $booking->days_attended = date_diff($booking->start_date, $booking->end_date) ?>
                                            <tr>
                                                <td><?= ++$key ?></td>
                                                <td><?= date_format($booking->created, "d/m/Y h:m:s") ?></td>
                                                ...
                                                <td><?php
                                                    if ($booking->sale_id != $booking->user_id) {
                                                        echo number_format($booking->price - $booking->revenue - $booking->sale_discount - $booking->agency_discount);
                                                    } else {
                                                        echo number_format($booking->price - $booking->sale_discount - $booking->agency_discount);
                                                    }
                                                    ?></td>
                                                <td><?php
                                                    if (($booking->sale_id == $booking->user_id)) {
                                                        echo number_format($booking->sale_revenue + $booking->revenue - $booking->sale_discount - $booking->agency_discount);
                                                    } else {
                                                        echo number_format($booking->sale_revenue - $booking->sale_discount - $booking->agency_discount);
                                                    }
                                                    ?></td>
                                                <td class="actions">
                                                    <a type="button" class="btn btn-xs btn-primary"
