...
<div class="col-md-12 col-sm-12 col-xs-12">
    <div class="x_panel">
        <div class="x_title">
            <h2>Thêm tài khoản ngân hàng có xuất hóa đơn</h2>
            <div class="clearfix"></div>
        </div>
        <div class="x_content">
            <br/>
            ...
            if (isset($bank_invoice)) {
                echo $this->Form->control('bank_invoice.bank_name', [
                    'type' => 'textarea',
                    'escape' => false,
                    'class' => 'form-control',
                    'label' => 'Tên ngân hàng *',
                    'required' => 'required',
                    'default' => $bank_invoice['bank_name']
                ]);
                echo $this->Form->control('bank_invoice.account_name', [
                    'type' => 'text',
                    'class' => 'form-control',
                    'label' => 'Tên chủ tài khoản *',
                    'required' => 'required',
                    'default' => $bank_invoice['account_name']
                ]);
                echo $this->Form->control('bank_invoice.account_number', [
                    'type' => 'text',
                    'class' => 'form-control',
                    'label' => 'Số tài khoản *',
                    'required' => 'required',
                    'default' => $bank_invoice['account_number']
                ]);
                echo $this->Form->control('bank_invoice.bank_branch', [
                    'type' => 'text',
                    'class' => 'form-control',
                    'label' => 'Chi nhánh *',
                    'required' => 'required',
                    'default' => $bank_invoice['bank_branch']
                ]);
                echo $this->Form->control('bank_invoice.bank_logo', [
                    'type' => 'file',
                    'label' => 'Logo ngân hàng'
                ]);
                echo $this->Form->control('bank_invoice.bank_logo_edit', [
                    'class' => 'form-control',
                    'type' => 'hidden',
                    'value' => $bank_invoice['bank_logo_edit']
                ]);
            } else {
                echo $this->Form->control('bank_invoice.bank_name', [
                    'type' => 'text',
            ...
            ?>
            <?php if (isset($bank_invoice)): ?>
            <br>
                <div class="col-sm-3 col-sm-offset-3">
                    <?= $this->Html->image('/' . $bank_invoice['bank_logo'], ['alt' => 'thumbnail', 'class' => 'img-responsive']); ?>
                </div>
            <?php endif ?>
        </div>
    </div>
