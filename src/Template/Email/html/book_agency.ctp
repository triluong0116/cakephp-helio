...
switch ($booking->type) {
    ...
    case LANDTOUR:
        $item = 'landtour';
        $partner = 'Ban tổ chức LandTour';
        break;
}
?>
<h3>K&iacute;nh gửi qu&yacute; khách : <?= $booking->full_name ?></h3>
...
<?php if ($booking->type != LANDTOUR): ?>
    <p>Qu&yacute; kh&aacute;ch vui l&ograve;ng xuất tr&igrave;nh x&aacute;c nhận đặt <?= $item ?> n&agrave;y cho lễ t&acirc;n
        trước khi nhận ph&ograve;ng.X&aacute;c nhận đặt ph&ograve;ng Booking số : <strong><?= $booking->code ?></strong></p>
<?php else: ?>
    <p>Quý khách vui lòng xuất trình xác nhận đặt landtour cho đơn vị tổ chức trước chuyến đi.X&aacute;c nhận đặt landtour Booking số : <strong><?= $booking->code ?></strong></p>
<?php endif; ?>
<?php if (!$booking->combos): ?>
    <p><strong>T&ecirc;n <?= $item ?>: <?php
            if ($booking->vouchers) {
...
<table style="margin-left: auto; margin-right: auto;" border="1">
    ...
    <?php if ($booking->hotels): ?>
        <tr>
            <td>
                <p><em>Mã booking của ks (nếu c&oacute;): </em></p>
            </td>
            <td>
    ...
    </tr><?php if ($booking->hotels && $booking->hotels->is_special): ?>
        <tr>
            <td>
    ...
    <?php if (!$booking_rooms): ?>
        <?php if($booking->type != LANDTOUR): ?>
            <tr>
                <td>
                    <p><em>Số <?= ($booking->hotels || $booking->home_stays) ? 'phòng' : 'lượng' ?> :</em></p>
                </td>
                <td>
                    <p>&nbsp;<?php
                        if ($booking->vouchers) {
        ...
        <tr>
            <td>
                <p><em><?= $booking->type == LANDTOUR ? "Ngày đi:" : "Check in:" ?></em></p>
            </td>
            <td>
        ...
        <tr>
            <td>
                <p><em><?= $booking->type == LANDTOUR ? "Ngày về:" : "Check out:" ?></em></p>
            </td>
            <td>
...
<table style="margin-left: auto; margin-right: auto;" border="1">
    <tbody>
    <tr>
        <td><em>Lưu ý gửi CTV:</em></td>
        <td><?= $booking->note_agency ?></td>
    </tr>
    </tbody>
</table>
<?php if ($booking->hotels): ?>
    <p><em>Lưu &yacute;: Thời gian nhận ph&ograve;ng l&agrave; 14:00 v&agrave; trả ph&ograve;ng l&agrave; 12:00(trưa). Điểm
            đến c&oacute;</em> <em>thể y&ecirc;u cầu tiền cọc cho c&aacute;c chi ph&iacute; ph&aacute;t sinh
            1,000,000VND-2,000,000VND/đ&ecirc;m.</em></p>
<?php endif; ?>
